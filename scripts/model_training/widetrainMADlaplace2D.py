...
class linear_branch_DeepONet(nn.Module):
    def __init__(self, branch_features, trunk_features, common_features):
        ...
        # Define the branch network (for branch input)
        self.branch_net = nn.Sequential(
            nn.Linear(branch_features, 2000, bias=False),
        )
        
        # Define the trunk network (for trunk input)
...
# Create the model and wrap it using DataParallel for multi-GPU training
...
if os.path.exists(checkpoint_path):
    ...
    u.module.load_state_dict(checkpoint['model_state_dict'])  # Use .module to access the model's state dict
