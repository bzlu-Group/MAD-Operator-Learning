...
item = 1  # MAD1 or MAD2
checkpoint_path = f"models/MADlaplace2D{item}L_checkpoint_1_1.pth"  # Path to save/load model checkpoints
...
def generate_l_shape_inner_points(num_points):
    """
    Generate points uniformly distributed within the L-shaped region.
    Args:
        num_points (int): Number of points to generate.
    Returns:
        l_shape_inner_points (Tensor): A tensor of points inside the L-shape.
    """
    ...
    # Filter out points outside the L-shaped region (excluding top-right corner)
...
# Generate grid points inside the L-shaped region
...
class linear_branch_DeepONet(nn.Module):
    def __init__(self, branch_features, trunk_features, common_features):
        ...
        Args:
            ...
            common_features (int): Number of common features shared between branch and trunk networks.
        """
...
# Initialize model and wrap it with DataParallel for multi-GPU support
...
def l_label(u, inputs1):
    ...
    # Prepare trunk and branch inputs for the model
    trunk_batch = trunk.repeat(num_rows, 1)
    utrue_batch = inputs1[:,4*(num_points-1):].reshape(-1, 1)  # True values for the function
    branch_batch = inputs1[:, 0:4*(num_points-1)].unsqueeze(1).repeat(1, 1976, 1).view(-1, 4*(num_points-1))
...
file_path = f"data/MADlaplace2D{item}L_{num_functions, num_points}_1.txt"
...
model_path = f"models/MADlaplace2D{item}L_{num_functions, num_points}_1_1.pth"  # Model save path
