...
class TestDataset(Dataset):
    def __init__(self, file_path):
        """
        Read the test data from a file, where each line contains a set of values.
        Each line will be split into individual floating-point values.
        """
        with open(file_path, 'r') as file:
            lines = file.readlines()
        # Store data as a list of lists of floating point values
    ...
    def __getitem__(self, index):
        """
        Retrieve a sample at the given index and return it as a tensor.
        """
        return torch.tensor(self.data[index], dtype=torch.float32)

# Load test dataset and create DataLoader for batching
test_dataset = TestDataset(test_file_path)
batch_size = 50  # Set batch size based on the size of the test dataset
...
u = linear_branch_DeepONet(4 * (num_points - 1), 2, 110).to(device)  # Create the model and move it to the device (GPU/CPU)
